let i = 6;
        ...
        var RENDERER = {
            ...
            changeAngle: function (event) {
                ...
                this.angleY =
                    ((this.width / 2 - x) / this.width) * 2 * this.MAX_ROTATION_ANGLE;
                this.angleX =
                    ((this.height / 2 - y) / this.height) * 2 * this.MAX_ROTATION_ANGLE;
            },
            ...
            render: function () {
                ...
                if (--this.fireworkInterval == 0) {
                    ...
                    this.maxFireworkInterval =
                        this.getRandomValue(this.FIREWORK_INTERVAL) | 0;
                    this.fireworkInterval = this.maxFireworkInterval;
                }
            }
        };
        var CAMERA = {
            ...
            getCompensatedParameters: function (point) {
                ...
                if (
                    x + radius < 0 ||
                    x - radius > this.renderer.width ||
                    y + radius < 0 ||
                    y - radius > this.renderer.height
                ) {
                    return null;
                }
        ...
        BASE.prototype = {
            ...
            getAxis2D: function () {
                ...
                this.parameters.color = this.COLOR.replace("%h", this.hue).replace(
                    "%l",
                    this.parameters.luminance
                );
        ...
        FIREWORK.prototype = new BASE({
            ...
            init: function () {
                var axis = this.renderer.getAxis3D(
                    Math.PI * Math.random(),
                ...
                switch (this.status) {
                    case 0:
                        ...
                        if (axis) {
                            ...
                            context.fillStyle = axis.color;
                            context.globalAlpha =
                                this.maxCount - this.count <= this.THRESHOLD
                                    ? Math.max(
                                        0,
                                        1 -
                                        (this.THRESHOLD - this.maxCount + this.count) /
                                        this.THRESHOLD
                                    )
                                    : 1;
                    ...
                    case 1:
                        if (--this.waitCount <= 0) {
                            ...
                            for (var i = 0, length = this.PARTICLE_COUNT; i < length; i++) {
                                this.particles.push(
                                    new PARTICLE(this.camera, this.x, this.y, this.z, this.hue)
                                );
                            }
                        }
                        return true;
